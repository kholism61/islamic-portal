...
if (!offline[articleId]) {
  ...
  localStorage.setItem(
    ...
    JSON.stringify(offline)
      ...
      /* ========= SEO ========= */
      document.title = `${data.judul} | Mimbar Ilmu Islam`;
...
if (schemaScript && data) {
  ...
  const schemaData = {
    ...
    "publisher": {
      ...
      "logo": {
        "@type": "ImageObject",
        "url": "https://httpsjovial-ganache-3b20a0.my.id/assets/images/logo.png"
      }
    },
...
// =====================
// TRANSLATE ARTICLE (GLOBAL)
// =====================

function translateTo(lang) {

  localStorage.setItem("articleLang", lang);

  document.querySelectorAll(".article-translate button")
    .forEach(btn => {
...
if (offlineBtn) {
  ...
  offlineBtn.addEventListener("click", () => {
    if (!articleId || !window.articlesData?.[articleId]) return;

    const offline =
      JSON.parse(localStorage.getItem("offlineArticles")) || {};

    offline[articleId] = window.articlesData[articleId];
...
function updateReaderStats() {
  ...
  // kalau belum ada bacaan → sembunyikan widget
  if (ids.length === 0) {
    section.style.display = "none";
    return;
  }

  // kalau ada bacaan → tampilkan
  section.style.display = "block";
