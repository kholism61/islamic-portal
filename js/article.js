...
if (articlesContainer && typeof articlesData !== "undefined") {
 ...
 const articleKeys = Object.keys(articlesData).sort((a, b) => {
  ...
  : article.judul}</h3>
      ...
      <a href="/article?id=${id}" class="read-more">Baca Selengkapnya</a>
  ...
  if (!featuredSection || articleKeys.length === 0) {
    ...
    function renderFeatured(index) {
      ...
      featuredSection.innerHTML = `
        <a href="/article?id=${id}" class="featured-card fade">
...
if (tocList && contentEl && tocBox) {
  ...
  <a href="/article?id=${key}" class="related-link">
    <img 
...
function renderOfflineHome() {
  ...
  ids.slice(0, 3).forEach((id) => {
    ...
    card.innerHTML = `
      ...
      <span class="category">${article.kategori}</span>
      <h3>${article.judul}</h3>
      <a href="/article?id=${id}" class="read-more">
        Baca Offline
      </a>
    `;
...
function renderLastReading() {
  ...
  <div class="last-reading-card">
    ...
    <a href="/article?id=${id}" class="btn-premium">
      Lanjutkan Membaca
    </a>
  </div>
`;
...
(function () {
  ...
  container.innerHTML = scored.map(item => `
    <a href="/article?id=${item.id}" class="recommended-card">
